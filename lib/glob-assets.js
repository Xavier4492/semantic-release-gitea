...
const { isPlainObject, castArray, uniqWith, uniq, template } = require('lodash');
const dirGlob = require('dir-glob');
const globby = require('globby');
const debug = require('debug')('saithodev:semantic-release-gitea');

module.exports = async (context, assets) => {
  const { cwd } = context;
  return uniqWith(
    []
      .concat(
        ...(await Promise.all(
          // Process each asset to interpolate placeholders before globbing
          assets.map(async asset => {
            // Raw path ou pattern template (e.g. '*.tar.gz' ou '*-<%= nextRelease.version %>.tar.gz')
            const raw = isPlainObject(asset) ? asset.path : asset;
            // Apply lodash.template to raw
            const interpolated = template(raw)(context);

            // Wrap single glob definition in Array
            let glob = castArray(interpolated);
            ...
            if (isPlainObject(asset)) {
              if (globbed.length > 1) {
                // If asset is an Object with a glob in the `path` property that resolves to multiple files,
              ...
              // - `path` of the matched file if there is one, or the interpolated `path` (will be considered as a missing file)
              // - other properties of the original asset definition
              return { ...asset, path: globbed[0] || interpolated };
            }

            if (globbed.length > 0) {
              // If asset is a String definition, output each file matched
              return globbed;
            }
